  ...
  return (
    <>
      ...
      <div className="container page">
        ...
        {tab === 'dashboard' && isChief && (
          <DashboardTab cfis={cfis} students={students} />
        )}

        {tab === 'cfis' && isChief && (
          <CFIsTab cfis={cfis} students={students} onAddCFI={() => setAddCFIOpen(true)} onEditStudent={setEditStudent} onRefresh={loadData} />
        )}
      ...
      {addCFIOpen && (
        <AddCFIModal
          onClose={() => setAddCFIOpen(false)}
          onSave={async (email, password, name, cert) => {
            const { data, error } = await supabase.auth.admin ? null : { error: 'Use Supabase dashboard to add CFI accounts.' }
...
function CFIsTab({ cfis, students, onAddCFI, onEditStudent, onRefresh }) {
  ...
  return (
    <div>
      ...
      {cfis.length === 0 && <div className="empty">No instructors yet. Add CFIs via Supabase dashboard.</div>}
      {cfis.map((cfi, ci) => {
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <p style={{ fontSize: 14, color: '#6b6b66', lineHeight: 1.6, marginBottom: 16 }}>
          To add a new CFI, go to your <strong>Supabase dashboard</strong> → Authentication → Users → Invite user. Use their email and set the metadata:
        </p>
        <code style={{ display: 'block', background: '#f4f4f0', padding: 12, borderRadius: 8, fontSize: 12, marginBottom: 16 }}>
          {`{ "full_name": "CFI Name", "role": "cfi" }`}
        </code>
        <p style={{ fontSize: 13, color: '#6b6b66' }}>They will receive an email invite to set their password and log in.</p>
        <div className="modal-footer">
